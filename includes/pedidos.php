...
if(!empty($logoff) && $logoff == true):
  ...
  $dataEhora    = date('d/m/Y H:i', strtotime('+1 hour', strtotime(date('Y-m-d H:i:s'))));
  $ip           = get_client_ip();
...
if (isset($_POST) and isset($_POST['ped_remove']) and $_POST['ped_remove']) {
    $deletbanco->ExeDelete("ws_pedidos", "WHERE user_id = :userid AND id  = :id_ped", "userid={$userlogin['user_id']}&id_ped={$_POST['ped_remove']}");
    if($deletbanco->getResult()):
		header("Location: {$site}{$Url[0]}/pedidos");
	else:
		echo "<script>
		x0p('Opss...', 
...
<div style="background-color:#ffffff;" class="container margin_60"> 
  ...
  <div class="row">
        <div class="text-center" style="margin: 0 auto;">
         <!--   <button class="btn btn-success" data-toggle="modal" data-target="#modalInsertNew">Inserir Novo Pedido</button>
        </div> -->
  </div>
          ...
          if($view == 0):
            ...
            <tr id="<?=$codigo_pedido;?>" <?=($view == 0 ? "style='background-color: #34AF23;font-weight: bold;color:#ffffff'" : "");?>>
             ...
             <td>
              ...
              <td><form action="" method="post"><button class="btn btn-danger btn-xs" name="ped_remove" value="<?=$id;?>">Deletar</button></form></td>
            </tr>   
            <?php
          endforeach;
          $corD = '000000';
...
<div class="alert alert-info container margin_60">
 ...
 <!-- The Modal -->
  ...
  <!-- The Modal -->
  <div class="modal" style="margin-top: 80px;" id="modalInsertNew">
    <div class="modal-dialog">
      <div class="modal-content">
      
        <!-- Modal Header -->
       <div class="modal-header">
          <h4 class="modal-title">Inserir Novo Pedido</h4> 
          <button type="button" class="close" data-dismiss="modal">&times;</button>
        </div> 
        
        <!-- Modal body -->
        <div class="modal-body">
            <span>Insira novos pedidos no sistema.</span></br></br>
            <!-- Function Modal -->
            <div class="container-fluid" id="content_InsertNew">
                <!--<div class="form-group">
                    <label for="ped_resumo">Resumo dos Pedidos</label>
                    <textarea class="form-control" name="ped_resumo" rows=4></textarea>
                </div>-->
                <div class="form-group">
                    <label for="item_name">Nome do Produto</label>
                    <input type="text" class="form-control" name="item_name" placeholder="Pizza de Mussarela"/>
                </div>
                <div class="form-group">
                    <label for="item_addtg">Adicionais Gratis</label>
                    <input type="text" class="form-control" name="item_addtg" placeholder="Morango, Chocolate"/>
                </div>
                <div class="form-group">
                    <label for="item_addt">Adicionais Gratis</label>
                    <input type="text" class="form-control" name="item_addt" placeholder="Camarão, Bacon"/>
                </div>
                <div class="form-group">
                    <?
                    $lerbanco->ExeRead('ws_formas_pagamento', "WHERE user_id = :userid", "userid={$userlogin['user_id']}");
                    if ($lerbanco->getResult()) {
                        ?>
                        <select class="form-control">
                            <option>Formas de Pagamento</option>
                            <?
                            foreach ($lerbanco->getResult() as $getDataPay) {
                                ?>
                                <option><?=$getDataPay['f_pagamento'];?></option>
                                <?
                            }
                            ?>
                        </select>
                ...
                <div class="form-group">
                    <label for="valor_troco">Valor Troco</label>
                    <input class="form-control" name="valor_troco" data-mask="#.##0,00" data-mask-reverse="true" placeholder="0,00"/>
                </div>
                <div class="form-group">
                    <select class="form-control">
                        <option>Opção Delivery</option>
                        <option value="true">Sim</option>
                        <option value="false">Não</option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="valor_troco">Valor Taxa</label>
                    <input class="form-control" name="taxa" data-mask="#.##0,00" data-mask-reverse="true" placeholder="0,00"/>
                </div>
                <div class="form-group">
                    <label for="telefone_empresa">Telefone da Empresa</label>
                    <input class="form-control" name="telefone_empresa" placeholder="(99) 99999-9999" data-mask="(00) 00000-0000" maxlength="15"/>
                </div>
            </div>
        </div>
        <script>listMotoboys();</script>
